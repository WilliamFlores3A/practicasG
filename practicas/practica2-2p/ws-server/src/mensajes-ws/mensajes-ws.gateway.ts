...
export class MensajesWsGateway implements OnGatewayConnection, OnGatewayDisconnect {
  ...
  listaDeUsuarios:string[]=['geancarlos','prueba2'];
  ...
  async handleConnection(client: Socket) {
    ...
    try {
      ...
      throw new BadRequestException('prueba2');
      payload= token;
    ...
    // client.join('ventas');
    // console.log(`deportista conectado`, client.id)
  ...
  onMessageFromClient(client: Socket, payload:NewMessageDto){
    ...
    // Solo para el deportista que emite
    // client.emit('message-from-server', {
    //   fullname:'Yo',
    //   message:payload.message || 'nada'
    // })
    // a todos menos al deportista inicial
