...
export class DeportistaService {
  ...
  async findOneByName(name: string): Promise<Deportista> {
    const deportista = await this.deportistaRepository.findOneBy({ name });
    if (!deportista) throw new NotFoundException(`Deportista ${name} not found`);
    return deportista;
  }
